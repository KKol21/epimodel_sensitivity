...
class TargetCalc:
    ...
    def get_output(self, lhs_table: torch.Tensor, batch_size: int) -> Dict[str, torch.Tensor]:
        ...
        self.max_targets_output = {comp: torch.zeros(n_samples, device=device) for comp in self.max_targets}
        self.sup_targets_output = {comp: torch.zeros(n_samples, device=device) for comp in self.sup_targets}
        self.max_targets_finished = {comp: torch.BoolTensor(range(0, n_samples)).to(device) for comp in self.max_targets}
    ...
    def save_finished_indices(self, solutions, indices) -> None:
        ...
        for comp in self.max_targets:
            if all(self.max_targets_finished[comp]):
                continue
            self.max_targets_finished[comp][indices] = self.max_stopping_condition(comp=comp, last_diff=last_diff)
    ...
    def save_output_for_finished(self, solutions: torch.Tensor, indices) -> None:
        for comp in self.max_targets:
            ...
            if any(finished):
                ...
                self.max_targets_output[comp][indices[finished]] = \
                    torch.where(maxes == 0,
        ...
        if any(finished):
            for comp in self.sup_targets:
                self.sup_targets_output[comp][indices[finished]] = \
                    self.sup_metric(solutions=solutions[finished], comp=comp)

    def max_metric(self, solutions, comp) -> torch.Tensor:
        comp_max = torch.stack(
            [torch.max(self.model.aggregate_by_age(solution=solutions[i, :, :],
                                                   comp=comp))
