...
class R0Generator(R0GeneratorBase):
    ...
    def generate_transition_block(transition_param: float, n_states: int) -> torch.Tensor:
        ...
        trans_block[1:, :n_states - 1] = trans_block[1:, :n_states - 1].fill_diagonal_(transition_param * n_states)
        return trans_block
    ...
    def _get_v(self) -> None:
        ...
        for age_group in range(self.n_age):
            ...
            v[e_start:e_end, e_start:e_end] = self.generate_transition_block(params["alpha"], self.n_e)
            i_start = e_end
            i_end = i_start + self.n_i
            # Transition between infected states
            v[i_start:i_end, i_start:i_end] = self.generate_transition_block(params["gamma"], self.n_i)
    ...
    def _get_f(self, contact_mtx: torch.Tensor) -> torch.Tensor:
        ...
        # Infectivity for each infected state
